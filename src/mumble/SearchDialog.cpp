	...
	SearchResultItem(const SearchResult &result, unsigned int id, parent_t parent,
		...
		QString matchText =
			m_result.fullText.replace(m_result.begin, m_result.length,
									  QLatin1String("<b>") + m_result.fullText.midRef(m_result.begin, m_result.length) + QLatin1String("</b>"));
...
SearchDialog::SearchDialog(QWidget *parent) : QWidget(parent), m_itemDelegate(new SearchDialogItemDelegate()) {
	...
	toggleOptions->setIcon(QIcon(QString::fromLatin1("skin:config_basic.png")));
	...
	if (g.mw) {
		//QObject::connect(g.mw, &MainWindow::serverSynchronized, this,
		...
		addAction(g.mw->qaSearch);
	}
	if (g.sh) {
		QObject::connect(g.sh.get(), &ServerHandler::disconnected, this,
...
void SearchDialog::on_toggleOptions_clicked() {
	...
	g.s.searchOptionsShown = !searchOptionBox->isVisible();
...
void SearchDialog::on_searchResultTree_currentItemChanged(QTreeWidgetItem *c, QTreeWidgetItem *) {
	...
	if (item.getResult().type == SearchType::User) {
		...
		if (user) {
			...
			g.mw->pmModel->setSelectedUser(user->uiSession);
		}
	} else {
		...
		if (channel) {
			...
			g.mw->pmModel->setSelectedChannel(channel->iId);
		}
	}
}
...
SearchResult regularSearch(const QString &source, const QString &searchTerm, SearchType type, bool caseSensitive) {
	...
	if (startIndex >= 0) {
		...
		return SearchResult(startIndex, length, type, source, QString::fromLatin1(""));
	} else {
		// Not found
		return SearchResult();
	}
...
QString getChannelHierarchy(const Channel &channel, bool includeSource) {
	...
	const QString separator = g.s.qsHierarchyChannelSeparator;
...
void SearchDialog::search(const QString &searchTerm) {
	if (searchTerm.size() == 0 || g.uiSession == 0) {
	...
	const bool searchUsers    = g.s.searchForUsers;
	const bool searchChannels = g.s.searchForChannels;
	const bool caseSensitive  = g.s.searchCaseSensitive;
	const bool useRegEx       = g.s.searchAsRegex;
	...
	if (useRegEx) {
		...
		if (!regex.isValid()) {
			...
			m_searchFieldStyleWrapper.setBackgroundColor(QString::fromLatin1("#fc5555"));
...
void SearchDialog::contextMenuEvent(QContextMenuEvent *event) {
	if (g.mw && searchResultTree->currentIndex().isValid() && g.uiSession > 0) {
...
void SearchDialog::performDefaultAction(const SearchResultItem &item) {
	...
	if (item.getResult().type == SearchType::User) {
		...
		if (selectedChannel) {
			...
			switch (g.s.searchChannelAction) {
				case ChannelAction::NONE:
					break;
				case ChannelAction::JOIN:
