...
const QString MultiStyleWidgetWrapper::UNSET_COLOR     = QLatin1String("");
const QString MultiStyleWidgetWrapper::UNSET_SELECTOR  = QLatin1String("*");
...
void MultiStyleWidgetWrapper::updateStyleSheet() {
	...
	if (m_fontSize != UNSET_FONTSIZE) {
		styleSheet += QString::fromLatin1(" %1 { font-size: %2px; }").arg(m_fontSizeSelector).arg(m_fontSize);
	}
	if (m_backgroundColor != UNSET_COLOR) {
		styleSheet += QString::fromLatin1(" %1 { background-color: %2; }").arg(m_backgroundColorSelector).arg(m_backgroundColor);
	}
